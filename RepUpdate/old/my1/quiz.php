...
<body>
	...
	if (isset($_POST['quiz'])) {
		...
		$week=$_POST['week1'];
		$query=mysqli_query($con,"select * FROM quiz_question,course where course.course_code=quiz_question.course_code AND quiz_question.course_code='".$course_code."' AND quiz_question.week='".$week."'");
		$idd = array();
		...
		if ($number>0) {
			...
			$fina=array();
			while ($j<$number) {
				$query1=mysqli_query($con,"select * FROM quiz_question,course where course.course_code=quiz_question.course_code AND quiz_question.course_code='".$course_code."' AND quiz_question.week='".$week."' order by question_id");
