...
$query= mysqli_query($con,"select DISTINCT(student.reg_no),student.stud_name,result.enroll_code FROM student,enroll,result WHERE student.reg_no=enroll.reg_no and result.enroll_code=enroll.enroll_code  and enroll.course_code = '$cc'");
$a=0;
...
<script type="text/javascript">
	...
	function totaldiv(){
		...
		while(data[i]){
			var a=0;
		$('#main').append('<table class="table_template darklinks" id="tablet'+j+'" border="1"><thead><tr><th>Register Number</th><th>Student Name</th><th>Marks</th></tr></thead><tbody id="tabot'+j+'"></tbody></table>');//table start and table head
		while(a<maxi && i<l){
			$('#tabot'+j).append('<tr><td>'+data[i][0]+'</td><td>'+data[i][1]+'</td><td>'+data[i][2]+'</td>');//table row start and data[i][0],data[i][1],data[i][2].....
			i++;
			a++;
		}
		$('#tablet'+j).hide();
