...
<div id="content-wrapper">
  ...
  <div class="container-fluid">
    ...
    $c = 0;
    $date = date('y-m-d');
    $qry="SELECT department.dept_name,courses.course_name,courses.course_code FROM department,courses,enroll WHERE department.dept_id=courses.dept_id AND courses.course_code=enroll.course_code AND enroll.start_date<='$date' AND enroll.end_date>='$date' AND enroll.reg_no='$regno' GROUP BY courses.course_code ORDER BY department.dept_name";
    $sql=mysqli_query($con,$qry);
    if ($sql) {
      while ($res = mysqli_fetch_object($sql)) {
        $coursesD[$c][0] = $res->dept_name;
        $coursesD[$c][1] = $res->course_name;
        $coursesD[$c][3] = $res->course_code;
        switch ($coursesD[$c][0]) {
          case 'COMMERCE':
          $coursesD[$c][2]='com';
          break;
                ...
                for ($i=0; $i < $c; $i++) { 
                  ...
                  <div class="single-portfolio col-sm-4 all <?= $coursesD[$i][2] ?>">
                    <div class="relative">
                      ...
                      <a href="studentss.php?course=<?= $coursesD[$i][3] ?>"> 
                        <div class="middle">
